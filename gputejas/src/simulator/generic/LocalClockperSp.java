...
public class LocalClockperSp {
	
	long currentTime;
	static int stepSize;
	int tpc_number, sm_number, sp_number;

	public LocalClockperSp(int x, int y, int z) {
	...
	public static void systemTimingSetUp(SP[][][] cores)
...
//		System.out.println("LCM = " + LCM);
		...
		for(i = 0; i < SystemConfig.NoOfTPC; i++) {
			for(j = 0; j < TpcConfig.NoOfSM; j++) {
				for (k = 0; k < SmConfig.NoOfSP; k++) {
					cores[i][j][k].setStepSize(1);
				}
			}
		}
	}
